...
import cn.teleinfo.bidadmin.system.wrapper.MenuWrapper;
...
import cn.teleinfo.bidadmin.system.entity.RoleMenu;
import cn.teleinfo.bidadmin.system.mapper.MenuMapper;
import cn.teleinfo.bidadmin.system.service.IMenuService;
import cn.teleinfo.bidadmin.system.service.IRoleMenuService;
...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
	...
	@Override
	public List<MenuVO> routes(Integer parentId, String roleId) {
		List<Menu> allMenus = baseMapper.allMenu();
		List<Menu> roleMenus = baseMapper.roleTreeMenu(parentId,Func.toIntList(roleId));
//		List<Menu> roleMenus = baseMapper.roleMenu(parentId,Func.toIntList(roleId));
//		roleMenus.addAll(getChild(roleMenus,roleId));
	...
	public List<Menu> getChild(List<Menu> roleMenus, String roleId){
		List<Menu> all = new ArrayList<>();
		for(Menu menu:roleMenus){
			List<Menu> menus= baseMapper.roleMenu(menu.getId(),Func.toIntList(roleId));
			if (menus != null && menus.size() > 0) {
				all.addAll(menus);
				all.addAll(getChild(menus, roleId));
			}else{
				all.addAll(menus);
				continue;
			}
